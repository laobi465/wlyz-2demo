...
- 完成版本：v0.6.0
- 完成项：
  - [x] 双向工单（终端用户→开发者 + 开发者→管理员）
  - [x] 基础 CRUD + 状态机（0待处理→1处理中→2已回复→3已关闭）
  - [x] 工单分类（1换机申请 2充值问题 3卡密问题 4其他）
  - [x] 双 Controller（H5TicketController + DevTicketController）
  - [x] 前端工单管理页（双 Tab：收件箱 + 已提交 + 详情对话流 + 新建工单）
  - [x] 回复审计表（jicek_ticket_reply，仅 INSERT+SELECT）
  - [x] 5 份核心文档同步
- 备注：
  - H5 端前端页面待 H5 整体框架就绪后补全
  - 管理员端 Controller（处理开发者提交的工单）待管理员后台框架就绪后补全
